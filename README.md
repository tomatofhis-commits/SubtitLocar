# SubtitLocar (ローカルAI字幕システム) v0.10
...
## デスクトップ字幕オーバーレイ表示機能の使い方

本システムには、OBSなどの配信ソフトを経由せずに、ゲーム画面や動画プレイヤーなどの上に直接翻訳字幕を表示（オーバーレイ）させる機能が搭載されています。

### 1. オーバーレイ表示の有効化
設定画面を開き、以下の項目を設定します：
* **「ゲーム画面上に翻訳字幕をオーバーレイ表示」**にチェックを入れます。
* ゲーム中の誤クリックを防ぐため、オーバーレイ画面は標準でマウスクリックを透過する（クリックスルー）ようになっています。

### 2. 字幕位置の調整（ドラッグ移動）
1. 設定画面で**「字幕の位置を調整する (ドラッグ移動許可)」**にチェックを入れます。
2. 字幕エリアが強調表示され、一時的にクリックスルーが解除されます。
3. 字幕枠をマウスドラッグで任意の場所に移動させます。移動した位置は自動的に `settings.json` に保存されます。
4. 位置調整が終わったら、**「字幕の位置を調整する」のチェックを外して**元に戻します（クリックスルーが再適用されます）。

### 3. 表示スタイルの調整
設定画面のラジオボタンより、以下の項目を動的に変更できます：
* **オーバーレイの背景スタイル**: 背景なし（文字のみ） / 半透明背景あり
* **オーバーレイの枠幅**: 小(35%) / 中(50%) / 大(70%)。枠幅（サイズ小: 24px, 中: 36px, 大: 48px）に応じてフォントサイズが自動的に変化します。
* **文字・背景の透明度**: 0% / 30% / 60% の中から選択可能です。

*※ オーバーレイ機能は、マイクやシステム音声のキャプチャ翻訳データのみが表示対象となります。外部の連携テキスト（SecreAIなど）はオーバーレイ表示されません。*
